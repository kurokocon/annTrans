...
if (replace) {
  ...
  str(listFeatures);
  #cat(sprintf("Transferring associated features\n"));
  #for (i in 1:length(source@ranges@width)) {
#	if ((feature <- sub(".*Parent=(.*);*.*", "\\1", sourceData[i])) %in% listFeatures && feature != sourceData[i]) {
 #                 if (!(i %in% listLines)) {
  #                      cat(sprintf("Transferred line %d from source: child of %s\n", i, feature));
   #                     output <- c(output, source[i]);
    #                    feature <- sub(".*Name=(.*);*.*", "\\1", sourceData[i]);
#			listLines <- c(listLines, i);
 #                 }
  #              }
